...
##  [ 2022년 10월 12일 ]
+ 캘린더 라이브러리 사용 결정 이유
    - 캘린더에서 구현해야하는 기능들이 라이브러리(FullCalendar)에는 있음
    - 가독성이 좋음
...
+ 회의 내용
    - DB 저장 시 장점과 단점
        - 장점
            - 완성했던 로그인 페이지와 진행 중이였던 회원가입 페이지를 사용할 수 있음
            - 서버만 연결이 되면 유지 보수 탁월
            - 유효성 검사에 좋음
        - 단점
            - 유지보수 비용이 든다
    ...
    - 파일 저장 시 장점과 단점
        - 장점
            - 회원가입페이지에 드는 시간을 줄일 수 있음
            - 프로젝트 완성 시간을 줄일 수 있음
            - 사용자가 파일을 지우지만 않으면 정보는 계속적으로 불러올 수 있음
            - 추가 유지보수 비용이 들지 않음
        - 단점
            - 로그인, 회원가입 페이지가 없어짐으로써 주요 기능이 줄어들었음
    - <b>결정 내용</b>
        - 데이터들을 DB 저장이 아닌 파일 저장으로 진행하기로 결정
        - DB 저장이 필요한 로그인, 회원가입은 없애기로 결정(파일 저장을 하게 되면 사용자의 데스크탑에 내용이 저장되기 때문에 해당 기능들이 필요 없음)
...
+ 어려웠던 부분
    ...
    - 최기룡
        - 로그인 기능 넣기
        - 회원가입 페이지에 있는 중복 체크가 어려웠음 
        - 회원가입 react-hook-form 커스텀하기
