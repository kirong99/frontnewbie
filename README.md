# 프뉴비(201930216 배다슬, 201840231 최기룡 , 201540218 선정인)
+ 팀명 : 프뉴비
+ 이름 : 배다슬(팀장: 디자인, 개발), 최기룡(팀원: 개발), 선정인(팀원: 디자인,)
+ 졸업작품 소개 사이트 : https://das0166.github.io/frontnewbie/
+ 졸업작품 주제 : 일정 관리 웹 사이트
+ 작품 소개
    - 작품명 : 나의 하루를 책임질 '투데이'
    - 개발환경 : <img src="https://img.shields.io/badge/HTML5-E34F26?style=for-the-badge&logo=HTML5&logoColor=white"> <img src="https://img.shields.io/badge/CSS3-1572B6?style=for-the-badge&logo=CSS3&logoColor=white"> <img src="https://img.shields.io/badge/React-61DAFB?style=for-the-badge&logo=React&logoColor=black"> <img src="https://img.shields.io/badge/JavaScript-F7DF1E?style=for-the-badge&logo=JavaScript&logoColor=white">
    - 작품 소개 : 오늘 해야할 일을 확인할 수 있는 투두 기능 및 캘린더 기능이 있는 사이트
    - 작품의 특징 : 캘린더 기능에 일정 추가까지 가능하여 제목, 시간, 장소 등을 작성할 수 있음
---
##  [ 2022년 11월 16일 ]
+ 어려웠던 부분
    - 배다슬
        - 데이터를 불러오지 못하는 오류로 인하여 어려움을 겪었음
        - 캘린더 일정을 어떻게 수정, 삭제할지 찾아보는데 시간이 걸렸음 -> 모달로 결정(confirm으로 하기엔 어려울 것을 보임)
    - 최기룡
        - 다크모드 적용시에 캘린더 라이브러리를 사용해 색상을 지정하는 것이 어려웠음(배경)
        - 일정 추가 시 날짜가 'YYYY-MM-DD'를 'YYYY년 MM월 DD일'로 변경하는데 어려움을 겪었음 -> DateFormat과 Locale을 사용하였음

+ 22일까지 할 일
    - 배다슬
        - 캘린더 일정 수정 및 삭제(모달 생성)
    - 최기룡
        - 파일 저장 불러오기

##  [ 2022년 11월 9일 ]
+ 어려웠던 부분
    - 배다슬
        - 함수형 컴포넌트가 아닌 변수형 컴포넌트를 사용했기때문에 이 컴포넌트에서 todo 구현하는 방식을 공부하기 위해서 인프런 강의를 참고하여 todo를 구현하였음
    - 최기룡
        - 새로고침이나 페이지 이동했을 경우 저장했던 데이터가 삭제되었던 것을 해결하였음. 이를 해결위해서 useState 초기값 설정 및 useEffect를 활용하여 localstorage 안에 있는 data가 초기화되지 않도록 구현함 

+ 15일까지 할 일
    - 배다슬
        - todo css
        - 캘린더 일정 수정 및 삭제
    - 최기룡
        - 다크모드
##  [ 2022년 11월 2일 ]
+ 어려웠던 부분
    - 최기룡
        - 캘린더 일정 추가 기능을 구현할 때 배열을 만들어 데이터를 저장하는 부분이 어려웠음 -> state와 onCreate를 이용해 해결
        - 배열을 불러왔으나 datepicker를 이용한 날짜를 불러오는 것이 안되고 있음
        - 새로고침을 하면 배열이 사라짐 -> localstorage를 사용하여 해결중

+ 8일까지 할 일
    - 배다슬
        - 왼쪽 메모지에서 todo 리스트 추가
    - 최기룡
        - localstorage 이용하여 새로고침하여도 데이터 유지
        - 날짜 선택 시 날짜 value값 가져오기

##  [ 2022년 10월 26일 ]
+ 어려웠던 부분
    - 배다슬
        - 오른쪽 포스트잇 컬러가 변경되지 않아 최기룡 팀원 도움을 받아 해결 -> 컴포넌트를 분리함으로써 해결
    - 최기룡
        - 메모지에 해당 일자에 있는 일정들 불러오기 -> display 활용하여 해결
        - 일정 추가(데이터는 추가가 됐지만 달력에 일정 표시가 되지 않음) -> 해결 중

+ 1일까지 할 일
    - 배다슬
        - 초기화 버튼 클릭 시 원래 설정되어 있던 컬러로 되돌아오기
    - 최기룡
        - 일정 추가 및 수정

##  [ 2022년 10월 19일 ]
+ 어려웠던 부분
    - 배다슬
        - 노트 색상 변경이 적용 되지 않아 어려웠음
    - 최기룡
        - 날짜 클릭해서 레이어 팝업 나오게하는 것
        - 배다슬 팀장과 같이 클릭했을 때 색상 변하게 하는 것

+ 25일까지 할 일
    - 배다슬
        - 최기룡 팀원과 함께 작업한 것을 바탕으로 메모지와 메모지 스티커 색상 변경까지 완료
    - 최기룡
        - 캘린더 공휴일 API 가져와서 넣기
        - 캘린더 일정 추가,삭제


##  [ 2022년 10월 12일 ]
+ 캘린더 라이브러리 사용 결정 이유
    - 캘린더에서 구현해야하는 기능들이 라이브러리(FullCalendar)에는 있음
    - 가독성이 좋음

+ 18일까지 할 일
    - 배다슬
        - 색상 변경 레이어 팝업으로 시도해보기
        - 팝업이 안된다면 현 상황에서 수정해보기
    - 최기룡
        - 캘린더 기능까지 완성
##  [ 2022년 10월 05일 ]
+ 회의 내용
    - DB 저장 시 장점과 단점
        - 장점
            - 완성했던 로그인 페이지와 진행 중이였던 회원가입 페이지를 사용할 수 있음
            - 서버만 연결이 되면 유지 보수 탁월
            - 유효성 검사에 좋음
        - 단점
            - 유지보수 비용이 든다
            - 회원가입페이지에 시간을 들여야함
    - 파일 저장 시 장점과 단점
        - 장점
            - 회원가입페이지에 드는 시간을 줄일 수 있음
            - 프로젝트 완성 시간을 줄일 수 있음
            - 사용자가 파일을 지우지만 않으면 정보는 계속적으로 불러올 수 있음
            - 추가 유지보수 비용이 들지 않음
        - 단점
            - 로그인, 회원가입 페이지가 없어짐으로써 주요 기능이 줄어들었음
    - <b>결정 내용</b>
        - 데이터들을 DB 저장이 아닌 파일 저장으로 진행하기로 결정
        - DB 저장이 필요한 로그인, 회원가입은 없애기로 결정(파일 저장을 하게 되면 사용자의 데스크탑에 내용이 저장되기 때문에 해당 기능들이 필요 없음)

+ 어려웠던 부분
    - 배다슬
        - 메인 페이지 '옵션 더보기' 버튼 클릭 시 옵션 내용 보이게 하기 -> 옵션 내용을 새로운 js파일로 생성하여 만드는 것으로 해결 완료
    - 최기룡
        - 로그인 기능 넣기
        - 회원가입 페이지에 있는 중복 체크가 어려웠음 
        - 회원가입 react-hook-form 커스텀하기

+ 11일까지 할 일
    - 배다슬
        - 색상 변경 레이어 팝업으로 시도해보기
        - 팝업이 안된다면 현 상황에서 수정해보기
    - 최기룡
        - 캘린더 라이브러리 안쓰고 기능까지 넣어서 완성 시키기

##  [ 2022년 09월 28일 ]
+ 회의 내용
    - 모니터 크기를 1920px에 맞춰서 작업 후 반응형으로 하기
    - 메인 내용들을 position:absolute가 아닌 margin-top으로 정중앙 설정하기
    - 로그인 페이지는 margin-top:120px
    - ```main {height: calc(100vh - 70px);}``` 보류
    - todo 페이지 노트만 크기 조정
    - 캘린더 width 는 1820px 정도로 하고 border 지우기
    - 캘린더 일정이 있을때는 일자 밑에 밑줄 쳐지는 느낌으로 하기
    - 일정 추가 및 필요한 버튼은 오른쪽에 위치해두기

+ 어려웠던 부분
    - 최기룡
        - 캘린더 페이지를 사용할때 내장 되어 있는 라이브러리 사용하였음(별도 라이브러리를 사용하기때문에 필요한 기능인 일정 추가 등을 수정하는것에 어려움을 느꼈음)
        - 회원가입 관련 라이브러리를 이해하는데 시간이 오래 걸렸음

+ 4일까지 할 일
    - 배다슬
        - 과제(포스터, ppt) 초기 시안 잡기
        - 메인 페이지 버튼 클릭시 왼쪽에 색상 변경하는 기능이 나올 수 있게 수정
        - 로그인 기능 추가 시도

    - 최기룡
        - 회원가입 페이지 기능 넣기(비밀번호 확인, 아이디 중복확인) / DB 필요한 기능은 마지막에 하기(회원가입 완료, 생년월일 설정)
        - 캘린더 기능(일정 추가)
        - 과제(포스터, ppt) 초기 시안 잡기

##  [ 2022년 09월 21일 ]
+ 회의 내용
    - 푸터 보류
    - 로그인 페이지
        - 소셜로그인(카카오, 네이버), 일반 로그인
    - 회원가입 페이지에 들어갈 내용
        - 이름, 이메일(중복 확인), 생년월일

+ 25일까지 할 일
    - 배다슬
        - 헤더 완성
        - 전반적인 React 학습
        - 메인 페이지 틀 잡기
        - 로그인 페이지 완성(기능 제외)

    - 최기룡
        - 캘린더 라이브러리 적용 시도
        - 전반적인 React 학습
        - 회원가입 페이지 완성(기능 제외)

##  [ 2022년 09월 14일 ]
+ 로고 완성  
    ![홈화면1](/src/image/today_logo.png)

+ Router Dom 사용하여 페이지 이동
+ 팀원과 Git repository 연동
+ 작품 기술서 최종 제출 완료


##  [ 2022년 09월 07일 ]
+ 팀규칙
    - 참여 : 팀 회의는 100% 참석하기, 결정된 참석 일정 일방적으로 불참하지 않기
    - 회의 : 일주일에 최소 1번 진행
    - 의견 : 자신의 의견은 자유롭게 얘기하기
    - 팀장 : 공평하게 진행될 수 있게 자유로운 분위기 조성
+ 작품기획(브레인스토밍)
    - 현재 상황
        - 일정 관리 및 Todo 웹 사이트를 만들고자 함
        - 프론트엔드로만 구성되어 있어 서버 및 DB 숙련도 부족
        - HTML, CSS에 특화된 팀원들로 구성되어 있기 때문에 기초적인 틀 개발은 문제 없음
        - React 및 JavaScript 숙련도 부족
        - 다른 사례들(캘린더, Toggl, Todoist)과 차별점을 두어야 함
    - 예상되는 결과물
        - 문제해결을 위한 실행 방법 : 공부를 병행하며 프로젝트 진행 예정, 벤치마킹을 진행하며 차별점을 구상할 예정 
        - 필요한 지식 및 기술 : HTML, CSS, React, JavaScript, DB
        - 예상되는 결과물 : Todo, 캘린더, 일정 관리
        - 회로도  
        ![회로도](src/image/today_schematic.png)
+ 예상 결과물
    - 홈화면
    ![홈화면1](src/image/today_main.png)
    ![홈화면2](src/image/today_colorEdit.png)
    - 캘린더
    ![캘린더](src/image/today_calendar.png)
    - 로그인
    ![로그인](src/image/today_login.png)

+ 기능
    + 홈화면<br>
         &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;**왼쪽 메모지**
        + 오늘 할 일이 보여짐(Todo)
        + today 왼쪽에 있는 edit 버튼을 누르면 메모지, 글 색상 등을 변경할 수 있으며 할 일을 추가할 수 있음
        + 화살표를 누르면 다음 날에 할 일을 확인할 수 있음<br> **오른쪽 메모지**
        + 포스트잇에는 오늘 날짜에 추가한 일정들이 나옴
    + 캘린더
        + 연도, 월, 주 단위로 캘린더를 볼 수 있음
        + 해당 날짜에 등록한 일정이 표시 됨
        + 생일이나 공휴일 등 주요 기념일 표시 됨
        + 오늘이 아닌 다른 날짜의 일정을 추가 할 수 있음
    + 로그인
        + 소셜 로그인도 가능


##  [ 2022년 08월 31일 ]
+ 팀명 : 프뉴비
+ 이름 : 배다슬(팀장: 디자인, 개발), 최기룡(팀원: 개발)
+ 졸업작품 소개 사이트 : https://das0166.github.io/frontnewbe/
+ 졸업작품 주제 : 일정 관리 웹 사이트
+ 작품 소개
    - 작품명 : 나의 하루를 책임질 '투데이'
    - 개발환경 : <img src="https://img.shields.io/badge/HTML5-E34F26?style=for-the-badge&logo=HTML5&logoColor=white"> <img src="https://img.shields.io/badge/CSS3-1572B6?style=for-the-badge&logo=CSS3&logoColor=white"> <img src="https://img.shields.io/badge/React-61DAFB?style=for-the-badge&logo=React&logoColor=black"> <img src="https://img.shields.io/badge/JavaScript-F7DF1E?style=for-the-badge&logo=JavaScript&logoColor=white"> <img src="https://img.shields.io/badge/MySQL-4479A1?style=for-the-badge&logo=MySQL&logoColor=white">
    - 작품 소개 : 오늘 해야할 일을 확인할 수 있는 투두 기능 및 캘린더 기능이 있는 사이트
    - 작품의 특징 : 캘린더 기능에 일정 추가까지 가능하여 제목, 시간, 장소 등을 작성할 수 있음
